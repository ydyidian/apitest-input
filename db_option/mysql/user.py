...
from utils.enc.security_aes import generate_token


class User(object):
    CHARS = ascii_lowercase + "-_" + ascii_uppercase

    def __init__(self, is_vip: int = 1, expire: str = None):
        self.user_id = f"{self._gen_id('U')}"
        self.album_id = f"{self._gen_id('A')}"
        self.album_name = PropUtils.gen_rand_name()
        self.album_code = f"{self._gen_id('C')}"
        self.nick_name = self.album_name[1:]
        self.open_id = self._gen_extra_id()
        self.union_id = self._gen_extra_id()
        self.md5_union_id = md5(self.union_id.encode()).hexdigest().upper()
        self.app_id = "wx877eef35efdc2df0"  # appid的规则待确认
        self.tel_no = PropUtils.gen_rand_tel_no()
    ...
    def _gen_id(self, prefix: str = ""):
        """
        自动生成id
        :param prefix: 前缀，A：相册，U：user ""
        :return: 自动生成的id
        """
        return f"{prefix}{datetime.now().strftime('%Y%m%d%H%M%S%f')}{random() * 1000:04.0f}"

    def _gen_extra_id(self):
        return "".join([choice(self.CHARS) for x in range(30)])

    def gen_token(self, timestamp, terminal_type, secret_key):
        content = f"{self.user_id}_{timestamp}_{terminal_type}"
        return generate_token(secret_key, content)
        # 调用java编译后
        # process = subprocess.Popen(
        #     f'cd {fp.basePath}/db_option/mysql && java SecurityAES {content} {secret_key}',
        #     shell=True,
        #     stdout=subprocess.PIPE
        # )
        # out_ = process.stdout.read()
        # if out_:
        #     token = out_.decode().strip()
        #     return token
        # else:
...
class PropUtils(object):
    ...
    LAST_NAMES = ["一", "乙", "二", "十", "丁", "厂", "七", "卜", "人", "入", "八", "九", "几", "儿", "了", "力", "乃", "刀", "又", "三",
    ...
    def gen_rand_name(cls):
        return choice(cls.FIRST_NAMES) + choice(cls.LAST_NAMES) + (choice(cls.LAST_NAMES) if randint(0, 1) else "")
...
class UserData(object):
    ...
    @classmethod
    def insert_album_info(cls, *users):
        album_info = (
            "INSERT INTO `tb_album_info`("
            "`c_album_id`, "
            "`c_album_name`, "
            "`c_album_icon`, "
            "`c_album_desc`, "
            ...
            "'亲们,关注我的微商相册?\n\n不仅第一时间查看上新动态?\n\n还能轻松一键转图朋友圈?', "
            "%s, "
            "'https://xcimg.szwego.com/code_C202008051640571920009556_20200808170711.png', "
            "'https://xcimg.szwego.com/minicode_C202008051640571920009556_20200808170715.png', "
            "NULL, "
            "'https://xcimg.szwego.com/minicode_A202008051640571920009228_20200805164057.png', "
            "NULL, "
            "'', "
            "'', "
            "NULL, "
    ...
    def insert_user_album(cls, *users):
        user_album = (
            "INSERT INTO `tb_user_album` ("
            "`c_album_id`, "
            "`c_user_id`, "
            "`n_role`, "
            "`n_item_del_switch`, "
            "`n_item_del_range_time`, "
            "`n_item_del_type`, "
            "`t_item_last_update_time`, "
            "`t_item_last_create_time`, "
            "`n_item_count`, "
            "`t_push_time`, "
            "`n_mini_type`, "
            "`t_create_time`, "
        ...
        cls.conn.execute_many(user_album, [(u.album_id, u.user_id) for u in users])

    @classmethod
    def insert_user_auth(cls, *users):
        user_auth = (
        ...
        cls.conn.execute_many(user_auth, [(u.user_id, u.open_id, u.union_id, u.app_id) for u in users])

    @classmethod
    def insert_user_token(cls, *users):
        user_token = (
        ...
        for terminal_type in ("android", "ios", "miniapp", "net"):
            secret_key = "".join([choice(ascii_letters) for x in range(16)])
            cls.conn.execute_many(
                ...
                [
                    (
                        ...
                        terminal_type,
                        secret_key,
                        u.gen_token(time_stamp, terminal_type, secret_key),
                    )
                    for u in users
                ],
            )
    ...
    @classmethod
    def insert_user_info(cls, *users):
        user_info = (
    ...
    @classmethod
    def insert_user_vip(cls, *users, expire_tm="9999-12-31"):
        user_vip = (
        ...
        cls.conn.execute_many(user_vip, [(u.user_id, u.album_id, u.is_vip, u.expire) for u in users])

    @classmethod
    def insert_album_config(cls, *users):
        album_config = (
            "INSERT INTO `tb_album_config` ("
            "`c_album_id`, "
            "`n_tag_sort`, "
            "`c_price_type`, "
            "`c_is_spot_price`, "
            ...
            "CURRENT_TIMESTAMP, "
            "CURRENT_TIMESTAMP, "
            '\'{"anti_fold": {"toCanvas": {"value": false, "themeType": 1, "removePrice": false, "withMiniCode": false}'
            ', "commentMode": false, "separateShare": false}, "c_diy_miniapp_icon": '
            '"https://xcimg.szwego.com/album_home_1.png", "c_diy_miniapp_version": 1}\')'
        )
        cls.conn.execute_many(album_config, [(u.album_id,) for u in users])

    @classmethod
    def insert_phone_login(cls, *users):
        phone_login = (
            "INSERT INTO `tb_user_phone_login` ("
            "`c_user_id`, "
            "`c_phone_num`, "
            "`c_login_pwd`, "
            "`n_is_valid`, "
        ...
        cls.conn.execute_many(
            phone_login, [(u.user_id, u.tel_no, md5(u.login_pwd.encode()).hexdigest()) for u in users]
        )

    @classmethod
    def insert_album_follows_attention(cls, *users):
        follow_attention = (
            "INSERT INTO tb_album_followers_fans_%s ("
            "c_user_id, "
            "c_album_id, "
            "c_fans_user_id, "
            "c_fans_shop_id, "
        ...
        follow_attention_fans = (
            "INSERT INTO tb_album_followers_attention_%s ("
            "c_user_id, "
            "c_album_id, "
            "c_fans_user_id, "
            "c_fans_shop_id, "
        ...
        cls.conn.execute_many(
            follow_attention, [(int(u.album_id[-1]), u.user_id, u.album_id, u.user_id, u.album_id) for u in users]
        )
        cls.conn.execute_many(
            follow_attention_fans, [(int(u.album_id[-1]), u.user_id, u.album_id, u.user_id, u.album_id) for u in users]
        )

    # ------------------------- 删除客户相关信息 -------------------------
    @classmethod
    def del_album_info(cls, *users):
        del_album_info = "delete from tb_album_info where c_album_id=%s"
        cls.conn.execute_many(del_album_info, [(u.album_id,) for u in users])

    @classmethod
    def del_user_album(cls, *users):
        del_user_album = "delete from tb_user_album where c_album_id=%s and c_user_id=%s"
        cls.conn.execute_many(del_user_album, [(u.album_id, u.user_id) for u in users])

    @classmethod
    def del_user_auth(cls, *users):
        del_user_auth = "delete from tb_user_auth where c_user_id=%s"
        cls.conn.execute_many(del_user_auth, [(u.user_id,) for u in users])

    @classmethod
    def del_user_token(cls, *users):
        del_user_token = "delete from tb_user_token where c_user_id=%s"
        cls.conn.execute_many(del_user_token, [(u.user_id,) for u in users])

    @classmethod
    def del_user_info(cls, *users):
        del_user_info = "delete from tb_user_info where c_user_id=%s"
        cls.conn.execute_many(del_user_info, [(u.user_id,) for u in users])

    @classmethod
    def del_user_vip(cls, *users):
        del_user_vip = "delete from tb_user_vip where c_user_id=%s"
        cls.conn.execute_many(del_user_vip, [(u.user_id,) for u in users])

    @classmethod
    def del_album_config(cls, *users):
        del_album_config = "delete from tb_album_config where c_album_id=%s"
        cls.conn.execute_many(del_album_config, [(u.album_id,) for u in users])

    @classmethod
    def del_phone_login(cls, *users):
        del_phone_login = "delete from tb_user_phone_login where c_user_id=%s"
        cls.conn.execute_many(del_phone_login, [(u.user_id,) for u in users])

    @classmethod
    def del_album_follows_attention(cls, *users):
        del_follower_fans = "delete from tb_album_followers_fans_%s where c_user_id=%s"
        del_follower_fans_attention = "delete from tb_album_followers_attention_%s where c_user_id=%s"
        cls.conn.execute_many(del_follower_fans, [(int(u.album_id[-1]), u.user_id) for u in users])
        cls.conn.execute_many(del_follower_fans_attention, [(int(u.album_id[-1]), u.user_id) for u in users])


if __name__ == "__main__":
    u = User()
    print(u.gen_token("20210824201021", "net", "4R0u6Q0lB6wv8KCZ"))
